    ...
    if df_original is not None:
        ...
        if missing_cols:
            ...
            if st.button("処理を開始"):
                ...
                st.write("ご利用のOSに合わせてダウンロードしてください。")

                # --- Windows用ダウンロードボタン (Shift-JIS) ---
                try:
                    csv_output_windows = df_new_output.to_csv(index=False, encoding='shift_jis')
                    st.download_button(
                        label="Windows用ダウンロード (Shift-JIS)",
                        data=csv_output_windows,
                        file_name="新規データ_統合結果_Windows.csv",
                        mime="text/csv"
                    )
                except UnicodeEncodeError as e:
                    st.warning(f"Windows用CSV（Shift-JIS）の生成中にエラーが発生しました。一部の文字が対応していません: {e}")
                    st.write("Shift-JISでのダウンロードはできませんでした。Mac用をご利用ください。")
                
                # --- Mac用ダウンロードボタン (UTF-8 BOMなし) ---
                # MacのExcelやNumbersはUTF-8 BOMなしで正常に開けることが多い
                csv_output_mac = df_new_output.to_csv(index=False, encoding='utf-8')
                st.download_button(
                    label="Mac用ダウンロード (UTF-8)",
                    data=csv_output_mac,
                    file_name="新規データ_統合結果_Mac.csv",
                    mime="text/csv"
                )

                st.write("※ Windows Excelの新しいバージョンではUTF-8 BOMあり (「UTF-8 BOMあり」ボタン) も文字化けしにくい場合があります。")
