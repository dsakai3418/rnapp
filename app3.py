...
if uploaded_file is not None:
    ...
    if df_original is not None:
        ...
        # ここを修正: clean_column_name関数が適用された後の想定されるカラム名
        camel_id_col = 'CamelLocationID (Camel拠点ID)' 
        ...
        if camel_id_col not in df_original.columns: # 正しいカラム名でチェック
            ...
            if st.button("処理を開始"):
                with st.spinner("データ処理中..."):
                    ...
                    for index, row in df_past.iterrows():
                        ...
                        camel_id = row[camel_id_col] # 正しいカラム名でアクセス
                    ...
                    # 新規データにIDを付与
                    df_new['Matched_CamelLocationID'] = np.nan
                ...
                st.dataframe(df_new_output[[address_col, tacoms_status_col, 'Matched_CamelLocationID']].head(10))
                ...
                st.download_button(
                    label="結果CSVをダウンロード",
                    data=csv_output,
                    file_name="新規データ_統合結果_CamelID.csv",
                    mime="text/csv"
                )
else:
